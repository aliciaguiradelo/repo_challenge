...
public class Programa {
	...
	public static void main(String[] args) throws ParseException, IOException {
		...
		do {
			...
			while (opcao != 1 && opcao != 2 && opcao != 3 && opcao != 18) {
				System.out.printf("\nOpção Inválida!\n");
			...
			if (opcao == 1) {
				...
				do {
					...
					if (indexUsuarioLogado != -1) {
						...
						if (usuarioLogado.getPapel().equals("comum")) {
							...
							while (opcao != 1 && opcao != 2 && opcao != 18) {
								System.out.printf("\nOpção Inválida!\n");
								System.out.printf("Digite novamente a opção desejada: ");
								opcao = scn.nextInt();
							}
													...
													if (indexEmpresa != -1) {
														...
														if (resposta == 'S') {
															...
															if(empresaJaExistente == 'N') {
																...
																System.out.println("Empresa já consta na sua lista de favoritos!");
															}
														}
								...
								if (categorias.size() > 0) {
									...
									for (Postagem p : postagens) {
										System.out.printf("ID: " + p.getId() + "\tTítulo: " + p.getTitulo()
												...
												+ p.getCategoria() + "\tCriado por: " + p.getUsuario().getNome()
												+ "\tLikes: " + p.getLikes() + "\n");
									}
								...
								if (postagens.size() > 0) {
									...
									if (indexPostagemGeral != -1) {
										...
										System.out.printf("ID: " + post.getId() + "\tTítulo: " + post.getTitulo()
												...
												+ sdf.format(post.getDate()) + post.getUsuario().getNome() + "\tLikes: "
												+ post.getLikes() + "\n\n");
										System.out.printf("ID: " + postUsuario.getId() + "\tTítulo: "
												...
												+ sdf.format(postUsuario.getDate()) + postUsuario.getUsuario().getNome()
												+ "\tLikes: " + postUsuario.getLikes() + "\n\n");
									} else {
										System.out.printf("Postagem não encontrada\n\n");
									}
								} else {
									...
									if (indexPostagemGeral != -1) {
										...
										for (Postagem p : postagens) {
											System.out.printf("ID: " + p.getId() + "\tTítulo: " + p.getTitulo()
													...
													+ p.getImgUrl() + "\tData: " + sdf.format(p.getDate())
													+ "\tCriado por: " + p.getUsuario().getNome() + "\tLikes: "
													+ p.getLikes() + "\n");
										}
									...
									if (resposta == 'S') {
										...
										if (indexPostagemGeral != -1) {
											...
											System.out.printf("ID: " + post.getId() + "\tTítulo: " + post.getTitulo()
													...
													+ post.getUsuario().getNome() + "\tLikes: " + post.getLikes()
													+ "\n");
								...
								if (empresas.size() > 0) {
									...
									if (indexEmpresa != -1) {
										...
										if (emp.getGovernancas().size() > 0) {
											...
											System.out
													.printf("\nNão existe uma governança cadastrada dessa empresa para serem atualizadas!");
										}
										...
										if (emp.getValores().size() > 0) {
											...
											System.out.printf("\nNão existe valores cadastrados dessa empresa para serem atualizados!");
										}
										...
										if (emp.getBalancos().size() > 0) {
											...
											System.out.printf("\nNão existe balanços cadastrados para essa empresa!\n\n");
										}
				...
				} while (opcao >= 1 && opcao <= 17 && usuarioLogado != null);
